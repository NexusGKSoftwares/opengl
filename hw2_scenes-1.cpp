...
//     auto transform_it = node.find("transform");
//     if (transform_it == node.end()) {
//         return F; // Return identity if no transformation
//     }

//     for (auto it = transform_it->begin(); it != transform_it->end(); it++) {
//         if (auto scale_it = it->find("scale"); scale_it != it->end()) {
//             Vector3 scale = Vector3{(*scale_it)[0], (*scale_it)[1], (*scale_it)[2]};
...
//             F = scale_matrix * F;
//         } else if (auto rotate_it = it->find("rotate"); rotate_it != it->end()) {
//             Real angle = static_cast<Real>((*rotate_it)[0]) * M_PI / 180.0; // Convert degrees to radians
//             Vector3 axis = normalize(Vector3{(*rotate_it)[1], (*rotate_it)[2], (*rotate_it)[3]});
            
//             Real cos_theta = std::cos(angle);
//             Real sin_theta = std::sin(angle);
//             Real one_minus_cos = 1 - cos_theta;
...
//             F = rotation_matrix * F;
//         } else if (auto translate_it = it->find("translate"); translate_it != it->end()) {
//             Vector3 translate = Vector3{(*translate_it)[0], (*translate_it)[1], (*translate_it)[2]};
...
//             auto up_it = lookat_it->find("up");
//             if (position_it != lookat_it->end()) {
//                 position = Vector3{(*position_it)[0], (*position_it)[1], (*position_it)[2]};
//             }
//             if (target_it != lookat_it->end()) {
//                 target = Vector3{(*target_it)[0], (*target_it)[1], (*target_it)[2]};
//             }
//             if (up_it != lookat_it->end()) {
...
//             lookat_matrix(0, 1) = u.x;
//             lookat_matrix(1, 1) = u.y;
//             lookat_matrix(2, 1) = u.z;
